...
\tikzset{weird fill/.style={append after command={
			\pgfextra
			\draw[sharp corners, fill=#1,fill opacity=1, draw=none]% 
			(\tikzlastnode)% 
...
%	\node [anchor=west] at (#5-#3,  #6+#4+0.35) {\texttt{#2}};
	...
	\path[draw,black,fill=#1,fill opacity=1,rounded corners=8pt] (#5, #6 + #4) -- (#5,#6) -- (#5+#3, #6) -- (#5+#3, #6 + #4) -- (#5, #6 + #4);
}
...
\begin{document}
		\begin{tikzpicture}
		\package[red!30](ectgui:14:14.5:0:0);
		\package[blue!30](generator\_dialog:5:2.5:0.3:0.2);	
